...
namespace Jah {
	...
	void EditorCamera::OnUpdate(Timestep ts)
	{
		...
		// Comment/Uncomment this for right click 3D rotation
		if (Input::IsMouseButtonPressed(JAH_MOUSE_BUTTON_RIGHT))
			MouseRotate(delta);
	...
	void EditorCamera::KeyboardPan(Timestep ts)
	{
		glm::vec3 move(0.0f);
		...
		if (glm::length(move) > 0.0f)
		{
			...
			float speed = m_Distance * 2.0f; // movement speed scales with zoom
			m_FocalPoint += move * speed * ts.GetSeconds();
		}
	}
